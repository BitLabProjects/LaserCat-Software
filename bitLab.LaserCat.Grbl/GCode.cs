...
namespace bitLab.LaserCat.Grbl
{
  unsafe public partial class Grbl
  {
    ...
    public const byte MODAL_GROUP_G0 = 0; // [G4,G10,G28,G28.1,G30,G30.1,G53,G92,G92.1] Non-modal
    public const byte MODAL_GROUP_G1 = 1; // [G0,G1,G2,G3,G38.2,G80] Motion
    public const byte MODAL_GROUP_G2 = 2; // [G17,G18,G19] Plane selection
    public const byte MODAL_GROUP_G3 = 3; // [G90,G91] Distance mode
    public const byte MODAL_GROUP_G5 = 4; // [G93,G94] Feed rate mode
    public const byte MODAL_GROUP_G6 = 5; // [G20,G21] Units
    public const byte MODAL_GROUP_G8 = 6; // [G43,G43.1,G49] Tool length offset
    public const byte MODAL_GROUP_G12 = 7; // [G54,G55,G56,G57,G58,G59] Coordinate system selection

    public const byte MODAL_GROUP_M4 = 8 ; // [M0,M1,M2,M30] Stopping
    public const byte MODAL_GROUP_M7 = 9 ; // [M3,M4,M5] Spindle turning
    public const byte MODAL_GROUP_M8 = 10; // [M7,M8,M9] Coolant control

    public const byte OTHER_INPUT_F = 11;
    public const byte OTHER_INPUT_S = 12;
    public const byte OTHER_INPUT_T = 13;
    // Define command actions for within execution-type modal groups (motion, stopping, non-modal). Used
    // internally by the parser to know which command to execute.

    // Modal Group G0: Non-modal actions
    public const byte NON_MODAL_NO_ACTION = 0; // (Default: Must be zero)
    public const byte NON_MODAL_DWELL = 1; // G4
    public const byte NON_MODAL_SET_COORDINATE_DATA = 2; // G10
    public const byte NON_MODAL_GO_HOME_0 = 3; // G28
    public const byte NON_MODAL_SET_HOME_0 = 4; // G28.1
    public const byte NON_MODAL_GO_HOME_1 = 5; // G30
    public const byte NON_MODAL_SET_HOME_1 = 6; // G30.1
    public const byte NON_MODAL_ABSOLUTE_OVERRIDE = 7; // G53
    public const byte NON_MODAL_SET_COORDINATE_OFFSET = 8; // G92
    public const byte NON_MODAL_RESET_COORDINATE_OFFSET = 9; //G92.1

    // Modal Group G1: Motion modes
    public const byte MOTION_MODE_SEEK = 0; // G0 (Default: Must be zero)
    public const byte MOTION_MODE_LINEAR = 1; // G1
    public const byte MOTION_MODE_CW_ARC = 2 ; // G2
    public const byte MOTION_MODE_CCW_ARC = 3 ; // G3
    public const byte MOTION_MODE_PROBE = 4; // G38.2
    public const byte MOTION_MODE_NONE = 5; // G80

    // Modal Group G2: Plane select
    public const byte PLANE_SELECT_XY = 0; // G17 (Default: Must be zero)
    public const byte PLANE_SELECT_ZX = 1; // G18
    public const byte PLANE_SELECT_YZ = 2; // G19

    // Modal Group G3: Distance mode
    public const byte DISTANCE_MODE_ABSOLUTE = 0; // G90 (Default: Must be zero)
    public const byte DISTANCE_MODE_INCREMENTAL = 1; // G91

    // Modal Group M4: Program flow
    public const byte PROGRAM_FLOW_RUNNING = 0; // (Default: Must be zero)
    public const byte PROGRAM_FLOW_PAUSED = 1; // M0, M1
    public const byte PROGRAM_FLOW_COMPLETED = 2; // M2, M30

    // Modal Group G5: Feed rate mode
    public const byte FEED_RATE_MODE_UNITS_PER_MIN = 0; // G94 (Default: Must be zero)
    public const byte FEED_RATE_MODE_INVERSE_TIME = 1; // G93

    // Modal Group G6: Units mode
    public const byte UNITS_MODE_MM = 0; // G21 (Default: Must be zero)
    public const byte UNITS_MODE_INCHES = 1; // G20

    // Modal Group M7: Spindle control
    public const byte SPINDLE_DISABLE = 0; // M5 (Default: Must be zero)
    public const byte SPINDLE_ENABLE_CW = 1; // M3
    public const byte SPINDLE_ENABLE_CCW = 2; // M4

    // Modal Group M8: Coolant control
    public const byte COOLANT_DISABLE = 0; // M9 (Default: Must be zero)
    public const byte COOLANT_MIST_ENABLE = 1; // M7
    public const byte COOLANT_FLOOD_ENABLE = 2; // M8

    // Modal Group G8: Tool length offset
    public const byte TOOL_LENGTH_OFFSET_CANCEL = 0; // G49 (Default: Must be zero)
    public const byte TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC = 1; // G43.1

    // Modal Group G12: Active work coordinate system
    // N/A: Stores coordinate system value (54-59) to change to.

    public const byte WORD_F =  0;
    public const byte WORD_I =  1;
    public const byte WORD_J =  2;
    public const byte WORD_K =  3;
    public const byte WORD_L =  4;
    public const byte WORD_N =  5;
    public const byte WORD_P =  6;
    public const byte WORD_R =  7;
    public const byte WORD_S =  8;
    public const byte WORD_T =  9;
    public const byte WORD_X =  10;
    public const byte WORD_Y =  11;
    public const byte WORD_Z =  12;
    // NOTE: When this struct is zeroed, the above defines set the defaults for the system.
    public struct gc_modal_t{
      public byte motion;        // {G0,G1,G2,G3,G38.2,G80}
      public byte feed_rate;     // {G93,G94}
      public byte units;         // {G20,G21}
      public byte distance;      // {G90,G91}
      public byte plane_select;  // {G17,G18,G19}
      public byte tool_length;   // {G43.1,G49}
      public byte coord_select;  // {G54,G55,G56,G57,G58,G59}
      public byte program_flow;  // {M0,M1,M2,M30}
      public byte coolant;       // {M7,M8,M9}
      public byte spindle;       // {M3,M4,M5}
    ...
    public struct gc_values_t{
      public float f;         // Feed
      public float[] ijk;    // I,J,K Axis arc offsets
      public byte l;       // G10 or canned cycles parameters
      public int n;       // Line number
      public float p;         // G10 or dwell parameters
      // public float q;      // G82 peck drilling
      public float r;         // Arc radius
      public float s;         // Spindle speed
      public byte t;       // Tool selection
      public float[] xyz;    // X,Y,Z Translational axes
    ...
    public struct parser_state_t{
      public gc_modal_t modal;
  
      public float spindle_speed;          // RPM
      public float feed_rate;              // Millimeters/min
      public byte tool;                 // Tracks tool number. NOT USED.

      public float[] position;       // Where the interpreter considers the tool to be at this point in the code
      public float[] coord_system;   // Current work coordinate system (G54+). Stores offset from absolute machine
                                             // position in mm. Loaded from EEPROM when called.  
      public float[] coord_offset;   // Retains the G92 coordinate offset (work coordinates) relative to
                                             // machine zero in mm. Non-persistent. Cleared upon reset and boot.    
      public float tool_length_offset;       // Tracks tool length offset value when enabled.
      ...
      public void init()
      {
        ...
        feed_rate = 0.0f;
        tool = 0;
        position = new float[N_AXIS];
        coord_system = new float[N_AXIS];
        coord_offset = new float[N_AXIS];
        tool_length_offset = 0;
      }
    };

    public struct parser_block_t{
    //   ushort command_words;  // NOTE: If this bitflag variable fills, G and M words can be separated.
    //   ushort value_words;

      public byte non_modal_command;
      public gc_modal_t modal;
      public gc_values_t values;
    ...
    public int MAX_LINE_NUMBER = 9999999 ;
    public byte AXIS_COMMAND_NONE = 0;
    public byte AXIS_COMMAND_NON_MODAL = 1 ;
    public byte AXIS_COMMAND_MOTION_MODE = 2;
    public byte AXIS_COMMAND_TOOL_LENGTH_OFFSET = 3; // *Undefined but required
    ...
    static bool gc_check_same_position(float[] pos_a, float[] pos_b) 
    {
      byte idx;
    ...
    byte gc_execute_line(char[] line) 
    {
      ...
      float[] coordinate_data = new float[N_AXIS]; // Multi-use variable to store coordinate data for execution
      float[] parameter_data = new float[N_AXIS]; // Multi-use variable to store parameter data for execution
      ...
      ushort command_words = 0; // G and M command words. Also used for modal group violations.
      ushort value_words = 0; // Value words. 
      ...
      byte word_bit = 0; // Bit-value for assigning tracking variables
      byte char_counter = 0;  
      char letter;
      float value = 0.0f;
      byte int_value = 0;
      ...
      while (line[char_counter] != 0) { // Loop until no more g-code words in line.
        ...
        char_counter++;
        if (read_float(line, ref char_counter, ref value) == 0) { return(STATUS_BAD_NUMBER_FORMAT); } // [Expected word value]
        ...
        int_value = (byte)trunc(value);
        mantissa =  (byte)System.Math.Round(100*(value - int_value)); // Compute mantissa for Gxx.x commands.
        ...
        switch(letter) {
          ...
          case 'G':
            ...
            bool perform4Or53 = false;
            bool perform80 = false;
            switch(int_value) {
              case 10: case 28: case 30: case 92: 
                // Check for G10/28/30/92 being called with G0/1/2/3/38 on same block.
                // * G43.1 is also an axis command but is not explicitly defined this way.
                if (mantissa == 0) { // Ignore G28.1, G30.1, and G92.1
                  if (axis_command != 0) { return(STATUS_GCODE_AXIS_COMMAND_CONFLICT); } // [Axis word/command conflict]
              ...
              case 43: case 49:
                ...
                } else { return(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [Unsupported G43.x command]
              ...
              case 54: case 55: case 56: case 57: case 58: case 59: 
                ...
                break;
              default: return(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G command]
            }
            if (perform4Or53)
            {
              ...
              switch (int_value)
              {
                ...
                case 28:
                  switch (mantissa)
                  {
                    ...
                    default: return (STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G28.x command]
                  }
                  mantissa = 0; // Set to zero to indicate valid non-integer G command.
                  break;
                case 30:
                  switch (mantissa)
                  {
                    ...
                    default: return (STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G30.x command]
                  }
                  mantissa = 0; // Set to zero to indicate valid non-integer G command.
                  break;
                ...
                case 92:
                  switch (mantissa)
                  {
                    ...
                    default: return (STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G92.x command]
                  }
                  mantissa = 0; // Set to zero to indicate valid non-integer G command.
                  break;
              }
              ...
              switch (int_value)
              {
                ...
                case 38:
                  switch (mantissa)
                  {
                    ...
                    // NOTE: If G38.3+ are enabled, change mantissa variable type to ushort.
                    ...
                    default: return (STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G38.x command]
                  }
                  mantissa = 0; // Set to zero to indicate valid non-integer G command.
                  break;
            ...
            if (mantissa > 0) { return(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [Unsupported or invalid Gxx.x command]
          ...
          case 'M':
            ...
            if (mantissa > 0) { return(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [No Mxx.x commands]
            switch(int_value) {
              ...
              case 7: case 8: case 9:
                ...
                switch(int_value) {      
                  case 7: if (!ENABLE_M7) break; gc_block.modal.coolant = COOLANT_MIST_ENABLE; break;
                  case 8: gc_block.modal.coolant = COOLANT_FLOOD_ENABLE; break;
                  case 9: gc_block.modal.coolant = COOLANT_DISABLE; break;
                }
                break;
              default: return(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported M command]
            }            
          ...
          default: 
            ...
            if (bit_istrue(value_words,bit(word_bit))) { return(STATUS_GCODE_WORD_REPEATED); } // [Word repeated]
            // Check for invalid negative values for words F, N, P, T, and S.
            // NOTE: Negative value check is done here simply for code-efficiency.
            if (( bit(word_bit) & (bit(WORD_F)|bit(WORD_N)|bit(WORD_P)|bit(WORD_T)|bit(WORD_S)) ) != 0) {
      ...
      */  
      ...
      if (axis_words != 0) {
        if (axis_command == 0) { axis_command = AXIS_COMMAND_MOTION_MODE; } // Assign implicit motion-mode
      }
  
      // Check for valid line number N value.
      if (bit_istrue(value_words,bit(WORD_N))) {
        // Line number value cannot be less than zero (done) or greater than max line number.
        if (gc_block.values.n > MAX_LINE_NUMBER) { return(STATUS_GCODE_INVALID_LINE_NUMBER); } // [Exceeds max line number]
      }
      ...
      if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
        ...
        if (axis_command == AXIS_COMMAND_MOTION_MODE) { 
          if ((gc_block.modal.motion != MOTION_MODE_NONE) || (gc_block.modal.motion != MOTION_MODE_SEEK)) {
            if (bit_isfalse(value_words,bit(WORD_F))) { return(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [F word missing]
          }
        }
      ...
      if (gc_block.non_modal_command == NON_MODAL_DWELL) {
        if (bit_isfalse(value_words,bit(WORD_P))) { return(STATUS_GCODE_VALUE_WORD_MISSING); } // [P word missing]
        bit_false(ref value_words,(ushort)bit(WORD_P));
      }
      ...
      if (axis_command == AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { // Indicates called in block.
        if (gc_block.modal.tool_length == TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC) {
          if ((axis_words ^ (1<<TOOL_LENGTH_OFFSET_AXIS)) != 0) { return(STATUS_GCODE_G43_DYNAMIC_AXIS_ERROR); }
        }
      }
      ...
      if ( bit_istrue(command_words,bit(MODAL_GROUP_G12)) ) { // Check if called in block
        if (gc_block.modal.coord_select > N_COORDINATE_SYSTEM) { return(STATUS_GCODE_UNSUPPORTED_COORD_SYS); } // [Greater than N sys]
        if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
          if (!(settings_read_coord_data(gc_block.modal.coord_select,coordinate_data))) { return(STATUS_SETTING_READ_FAIL); } 
        }
      }
      ...
      switch (gc_block.non_modal_command) {
        case NON_MODAL_SET_COORDINATE_DATA:  
          ...
          if (axis_words == 0) { return STATUS_GCODE_NO_AXIS_WORDS; }; // [No axis words]
          if (bit_isfalse(value_words,((1<<WORD_P)|(1<<WORD_L)))) { return(STATUS_GCODE_VALUE_WORD_MISSING); } // [P/L word missing]
          coord_select = (byte)trunc(gc_block.values.p); // Convert p value to int.
          if (coord_select > N_COORDINATE_SYSTEM) { return(STATUS_GCODE_UNSUPPORTED_COORD_SYS); } // [Greater than N sys]
          if (gc_block.values.l != 20) {
            if (gc_block.values.l == 2) {
              if (bit_istrue(value_words,bit(WORD_R))) { return(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G10 L2 R not supported]
            } else { return(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [Unsupported L]
          }
          bit_false(ref value_words,(ushort)(bit(WORD_L)|bit(WORD_P)));
      ...
      if (gc_block.modal.motion == MOTION_MODE_NONE) {
        ...
        if ((axis_words != 0) && (axis_command != AXIS_COMMAND_NON_MODAL)) { 
          return(STATUS_GCODE_AXIS_WORDS_EXIST); // [No axis words allowed]
        }
      ...
      // was explicitly commanded in the g-code block.
        ...
        if (gc_block.modal.motion == MOTION_MODE_SEEK) {
          ...
          if (axis_words == 0) { axis_command = AXIS_COMMAND_NONE; }

        // All remaining motion modes (all but G0 and G80), require a valid feed rate value. In units per mm mode,
        // the value must be positive. In inverse time mode, a positive value must be passed with each block.
        } else {      
          // Check if feed rate is defined for the motion modes that require it.
          if (gc_block.values.f == 0.0) { return(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [Feed rate undefined]
          ...
          switch (gc_block.modal.motion) {
            ...
            case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
              ...
              if (axis_words == 0) { return(STATUS_GCODE_NO_AXIS_WORDS); } // [No axis words]
              if ((axis_words & (bit(axis_0)|bit(axis_1))) == 0) { return(STATUS_GCODE_NO_AXIS_WORDS_IN_PLANE); } // [No axis words in plane]
              ...
              if ((value_words & bit(WORD_R))!=0) { // Arc Radius Mode  
                bit_false(ref value_words,(ushort)bit(WORD_R));
                if (gc_check_same_position(gc_state.position, gc_block.values.xyz)) { return(STATUS_GCODE_INVALID_TARGET); } // [Invalid target]
                ...
                */      
                ...
                if (h_x2_div_d < 0) { return(STATUS_GCODE_ARC_RADIUS_ERROR); } // [Arc radius error]
    
                // Finish computing h_x2_div_d.
                h_x2_div_d = -(float)System.Math.Sqrt(h_x2_div_d)/hypot_f(x,y); // == -(h * 2 / d)
                ...
                */  
                ...
                if ((ijk_words & (bit(axis_0)|bit(axis_1))) == 0) { return(STATUS_GCODE_NO_OFFSETS_IN_PLANE); } // [No offsets in plane]
                bit_false(ref value_words,(ushort)(bit(WORD_I)|bit(WORD_J)|bit(WORD_K)));  
          
                // Convert IJK values to proper units.
                if (gc_block.modal.units == UNITS_MODE_INCHES) {
                  for (idx=0; idx<N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
                    if ((ijk_words & bit(idx))!= 0) { gc_block.values.ijk[idx] *= MM_PER_INCH; }
                  }
                }         
                ...
                if (delta_r > 0.005) { 
                  if (delta_r > 0.5) { return(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.5mm
                  if (delta_r > (0.001*gc_block.values.r)) { return(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius
                }
              }
              break;
      ...
      bit_false(ref value_words,(ushort)(bit(WORD_N)|bit(WORD_F)|bit(WORD_S)|bit(WORD_T))); // Remove single-meaning value words. 
      if (axis_command != 0) { bit_false(ref value_words,(ushort)(bit(WORD_X)|bit(WORD_Y)|bit(WORD_Z))); } // Remove axis words. 
      if (value_words != 0) { return(STATUS_GCODE_UNUSED_WORDS); } // [Unused words]
      ...
      */ 
      ...
      if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
        ...
        Array.Copy(coordinate_data, gc_state.coord_system, coordinate_data.Length);
      }
      ...
      switch(gc_block.non_modal_command) {
        case NON_MODAL_SET_COORDINATE_DATA:    
          ...
          if (gc_state.modal.coord_select == coord_select) {
            copyArray(gc_state.coord_system, parameter_data); 
          }
          break;
        case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1: 
          ...
          if (axis_command != 0) {
            ...
            //#else
              mc_line(gc_block.values.xyz, -1.0f, false);
            //#endif
          }
          //#ifdef USE_LINE_NUMBERS
          //  mc_line(parameter_data, -1.0, false, gc_block.values.n); 
          //#else
            mc_line(parameter_data, -1.0f, false); 
          //#endif
            copyArray(gc_state.position, parameter_data);
          break;
        ...
        case NON_MODAL_SET_HOME_1:
          ...
          break;
        case NON_MODAL_SET_COORDINATE_OFFSET:
          copyArray(gc_state.coord_offset, gc_block.values.xyz);
          break;
        case NON_MODAL_RESET_COORDINATE_OFFSET: 
        ...
        if (axis_command == AXIS_COMMAND_MOTION_MODE) {
          switch (gc_state.modal.motion) {
            ...
            case MOTION_MODE_LINEAR:
              ...
              //#else
                mc_line(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate != 0);
              //#endif
              break;
            case MOTION_MODE_CW_ARC: case MOTION_MODE_CCW_ARC:
              //#ifdef USE_LINE_NUMBERS
          ...
          copyArray(gc_state.position, gc_block.values.xyz); // gc.position[] = target[];
        }
      }
      ...
      if (gc_state.modal.program_flow != 0) { 
        ...
        sys.auto_start = 0; // Disable auto cycle start. Forces pause until cycle start issued.
