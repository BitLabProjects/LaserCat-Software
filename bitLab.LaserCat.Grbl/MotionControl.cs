...
//#endif
{
  ...
  if (segments!=0) { 
    ...
    // all segments.
    if (invert_feed_rate !=0) { feed_rate *= segments; }
    ...
    */
    ...
    for (i = 1; i<segments; i++) { // Increment (segments-1).
      ...
      //#else
        mc_line(position, feed_rate, invert_feed_rate != 0);
      //#endif
  ...
  //#else
    mc_line(target, feed_rate, invert_feed_rate != 0);
  //#endif
}
...
void mc_dwell(float seconds) 
{
  //TODO
   ...
   //ushort i = floor(1000/DWELL_TIME_STEP*seconds);
...
{ 
  ...
  //protocol_buffer_synchronize();
  //byte auto_start_state = sys.auto_start; // Store run state
