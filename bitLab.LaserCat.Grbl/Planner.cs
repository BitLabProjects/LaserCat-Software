...
namespace bitLab.LaserCat.Grbl
{
	...
	unsafe public partial class GrblPlanner
	{
    ...
    public List<plan_block_t> block_buffer = new List<plan_block_t>();
      ...
      return block_index + 1;
		...
		*/
        ...
        if (block_index == block_buffer_tail) { mGrbl.st_update_plan_block_parameters(); }
			}
			else
		...
		public float plan_get_exec_block_exit_speed()
		{
			int block_index = plan_next_block_index(block_buffer_tail);
      ...
      block_buffer.Add(new plan_block_t(true));
        ...
        target_steps[idx] = (int)System.Math.Round(target[idx] * mGrbl.settings.steps_per_mm[idx]);
        ...
        delta_mm = (target_steps[idx] - pl.position[idx]) / mGrbl.settings.steps_per_mm[idx];
				unit_vec[idx] = delta_mm; // Store unit vector numerator. Denominator computed later.

				// Set direction bits. Bit enabled always means direction is negative.
        if (delta_mm < 0) { block_buffer[blockIdx].direction_bits |= mGrbl.get_direction_pin_mask(idx); }
			...
			if (feed_rate < mGrbl.MINIMUM_FEED_RATE) { feed_rate = mGrbl.MINIMUM_FEED_RATE; } // Prevents step generation round-off condition.
          ...
          feed_rate = (float)System.Math.Min(feed_rate, mGrbl.settings.max_rate[idx] * inverse_unit_vec_value);
          block_buffer[blockIdx].acceleration = (float)System.Math.Min(block_buffer[blockIdx].acceleration, mGrbl.settings.acceleration[idx] * inverse_unit_vec_value);
        ...
        block_buffer[blockIdx].max_junction_speed_sqr = (float)System.Math.Max(mGrbl.MINIMUM_JUNCTION_SPEED * mGrbl.MINIMUM_JUNCTION_SPEED,
                                     (block_buffer[blockIdx].acceleration * mGrbl.settings.junction_deviation * sin_theta_d2) / (1.0 - sin_theta_d2));
			}
			...
			GrblFirmware.copyArray(pl.previous_unit_vec, unit_vec); // pl.previous_unit_vec[] = unit_vec[]
			pl.previous_nominal_speed_sqr = block_buffer[blockIdx].nominal_speed_sqr;

			// Update planner position
      GrblFirmware.copyArray(pl.position, target_steps); // pl.position[] = target_steps[]
			...
			planner_recalculate();
      RaisePlannerBlocksChanged(EPlannerBlockChangedState.BlockAdded, new DblPoint3(target[0] * mGrbl.settings.steps_per_mm[0],
                                                                                    target[1] * mGrbl.settings.steps_per_mm[1],
                                                                                    target[2] * mGrbl.settings.steps_per_mm[2]));
		}                                                                               
		...
		public void plan_sync_position()
		{
			...
			for (idx = 0; idx < NutsAndBolts.N_AXIS; idx++)
			{
				pl.position[idx] = mGrbl.sys.position[idx];
			}
		}
      ...
      mGrbl.st_update_plan_block_parameters();
